# -*- coding: utf-8 -*-
"""A3_XAVIER_SOJA_V2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12qPAxxeSW8jatfNK7WTrZnCXcyzLMmDT
"""
...
df_soja_pivot.to_csv("csv/ibge_soja_transformado.csv", index=False, encoding="utf-8-sig")
...
# Seleção de variáveis quantitativas fundamentais
colunas_anual = ['Área plantada ou destinada à colheita', 'Quantidade produzida', 'Rendimento médio da produção', 'Preco_Real', 'Chuva_mm']
colunas_mensal = ['Preco_Real', 'Chuva_mm']

print("\n--- Resumo Estatístico - Base Anual (Extremo Oeste Baiano) ---")
resumo_anual = df_final[colunas_anual].describe().T
resumo_anual['median'] = df_final[colunas_anual].median() # Adiciona a mediana explicitamente
print(resumo_anual[['mean', 'median', 'std', 'min', '25%', '50%', '75%', 'max']])

print("\n--- Resumo Estatístico - Base Mensal (Sazonalidade Geral) ---")
resumo_mensal = df_mensal_final[colunas_mensal].describe().T
resumo_mensal['median'] = df_mensal_final[colunas_mensal].median()
print(resumo_mensal[['mean', 'median', 'std', 'min', '25%', '50%', '75%', 'max']])

#  VISUALIZAÇÃO INICIAL (Distribuição e Anomalias)


import matplotlib.pyplot as plt
import seaborn as sns

print("\n=== [2] VISUALIZAÇÃO INICIAL E IDENTIFICAÇÃO DE OUTLIERS ===")

# Criando Boxplots para identificar anomalias (Outliers)
plt.figure(figsize=(14, 5))

plt.subplot(1, 2, 1)
sns.boxplot(data=df_mensal_final, y='Preco_Real', color='skyblue')
plt.title('Distribuição e Outliers dos Preços da Soja (Mensal)')
plt.ylabel('Preço Real (R$)')

plt.subplot(1, 2, 2)
sns.boxplot(data=df_mensal_final, y='Chuva_mm', color='lightgreen')
plt.title('Distribuição e Outliers da Pluviometria de Barreiras (Mensal)')
plt.ylabel('Chuva Acumulada (mm)')

plt.tight_layout()
plt.savefig('csv/eda_boxplots_outliers.png')
plt.show()
...
plt.subplot(1, 2, 1)
sns.histplot(df_mensal_final['Preco_Real'], kde=True, color='blue', bins=20)
plt.title('Histograma - Preço da Soja')
plt.xlabel('Preço (R$)')

plt.subplot(1, 2, 2)
sns.histplot(df_mensal_final['Chuva_mm'], kde=True, color='green', bins=20)
plt.title('Histograma - Índice Pluviométrico')
plt.xlabel('Precipitação (mm)')

plt.tight_layout()
plt.savefig('csv//eda_histogramas.png')
plt.show()

# IDENTIFICAÇÃO DE PADRÕES E CORRELAÇÃO

print("\n=== [3] CORRELAÇÕES E TENDÊNCIAS TEMPORAIS ===")

# Análise de Correlação Linear (Pearson) e Não-Linear (Spearman) na base Anual do Oeste
print("\n--- Matriz de Correlação (Pearson) - Dados Anuais Integrados ---")
corr_pearson = df_final[colunas_anual].corr(method='pearson')
print(corr_pearson['Preco_Real'].round(3))

# Gráfico de Dispersão: Chuva Anual vs Rendimento (Produtividade)
plt.figure(figsize=(8, 5))
sns.regplot(data=df_final, x='Chuva_mm', y='Rendimento médio da produção', color='teal', marker='o')
plt.title('Dispersão: Pluviometria Anual vs Rendimento da Soja')
plt.xlabel('Chuva Acumulada Anual (mm)')
plt.ylabel('Rendimento Médio (kg/ha)')
plt.savefig('csv/eda_dispersao_chuva_rendimento.png')
plt.show()

# ANÁLISE DE SAZONALIDADE MENSAL

print("\n--- Calculando o Perfil Sazonal Mensal Histórico ---")

# Agrupando por mês para entender o ciclo agrícola e de mercado
sazonalidade = df_mensal_final.groupby('Mes')[['Preco_Real', 'Chuva_mm']].mean().reset_index()

# Criando gráfico de Linhas de Duplo Eixo (Preço vs Chuva ao longo dos meses do ano)
fig, ax1 = plt.subplots(figsize=(10, 6))

color = 'tab:blue'
ax1.set_xlabel('Mês do Ano')
ax1.set_ylabel('Preço Médio da Soja (R$)', color=color)
sns.lineplot(data=sazonalidade, x='Mes', y='Preco_Real', marker='o', color=color, linewidth=2.5, ax=ax1)
ax1.tick_params(axis='y', labelcolor=color)
ax1.set_xticks(range(1, 13))

ax2 = ax1.twinx()  # Compartilha o mesmo eixo X
color = 'tab:green'
ax2.set_ylabel('Precipitação Média Histórica (mm)', color=color)
sns.barplot(data=sazonalidade, x='Mes', y='Chuva_mm', color=color, alpha=0.4, ax=ax2)
ax2.tick_params(axis='y', labelcolor=color)

plt.title('Sazonalidade: Preço da Soja vs Ciclo Pluviométrico em Barreiras-BA')
fig.tight_layout()
plt.savefig('csv/eda_sazonalidade_mensal.png')
plt.show()

print("\nFase 2 (EDA)! Os gráficos foram salvos no diretório 'csv/'.")